...
export function HyperliquidPortfolioWidget({
  ...
  const activeAddress = customAddress || walletAddress || "";
  ...
  const totalRealizedPnl = account?.performanceHistory?.length
    ? account.performanceHistory[account.performanceHistory.length - 1].cumulativePnl
    : 0;
    ...
    return (
      <div className="ios-card p-6 border border-border/60 text-center relative overflow-hidden bg-gradient-to-b from-card/90 to-card/40">
        <div className="max-w-md mx-auto space-y-4 py-4">
          <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 flex items-center justify-center mx-auto shadow-lg shadow-emerald-500/10">
            <Zap className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-lg font-bold">Connect Hyperliquid L1 Account</h3>
            <p className="text-xs text-muted-foreground mt-1">
              Track your real-time open positions, perpetual margin health, and performance graph directly inside MarketWatch.
            </p>
          </div>
          ...
          <div className="flex items-center justify-center gap-2 pt-2 flex-wrap">
            ...
            <button
              onClick={() => setCustomAddress("0x5b38da6a701c568545dcfcb03fcb875f56beddc4")}
    ...
    <div className="space-y-6">
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="font-bold text-sm">Realized PnL Performance Trajectory</h4>
                <p className="text-[11px] text-muted-foreground">Cumulative performance calculated from Hyperliquid trade fills</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <div className="text-right">
                ...
                <span className={cn(
                  ...
                  totalRealizedPnl >= 0 ? "text-gain" : "text-loss"
                )}>
            ...
            <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-full overflow-visible">
              <defs>
                <linearGradient id="pnlGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor={totalRealizedPnl >= 0 ? "#10b981" : "#ef4444"} stopOpacity="0.3" />
                  <stop offset="100%" stopColor={totalRealizedPnl >= 0 ? "#10b981" : "#ef4444"} stopOpacity="0.0" />
                </linearGradient>
              </defs>
      ...
      >
        <div className="p-4 border-b border-border/40 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h4 className="font-bold text-sm">Open Perpetuals Positions</h4>
              <p className="text-[11px] text-muted-foreground">Live contracts traded on Hyperliquid L1 orderbook</p>
            </div>
          </div>
          <span className="text-xs font-semibold text-muted-foreground">
            {account?.positions?.length || 0} active
          </span>
        </div>
      ...
      {account?.spotBalances && account.spotBalances.length > 0 && (
        <motion.div
          ...
          className="ios-card p-5 border border-border/60 space-y-3"
        >
          <div className="flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 pt-1">
            {account.spotBalances.map((spot) => (
              <div key={spot.coin} className="p-3 rounded-2xl bg-muted/30 border border-border/40">
